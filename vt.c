...
write out the ASCII table as an int array, (like shown in class).

while(getchar() != EOF)
	printf(ACHII name of the value returned by getchar() and the hexadec	imal equivalence of that char)
*/
...
int main()
{
	...
	while ((a = getchar()) != EOF)
	{
			printf("ASCII NAME %s, Hexadecimal Equivalence %3x\n", &asciiname[4*a],a);
	}
}
